...
export function RoleBadge({ slug, name, size = 'sm' }: RoleBadgeProps) {
  ...
  const palette =
    (roleBadgeColors as Record<string, { bg: string; text: string }>)[slug] ??
    roleBadgeColors.DEFAULT;
...
export function StatusBadge({ status }: StatusBadgeProps) {
  const palette =
    (statusColors as Record<string, { bg: string; text: string; dot: string }>)[status] ??
  ...
  return (
    <View style={[styles.badge, styles.badgeSm, { backgroundColor: palette.bg }]}>
      ...
      <Text style={[styles.badgeText, styles.textSm, { color: palette.text }]}>{label}</Text>
    </View>
  );
}
